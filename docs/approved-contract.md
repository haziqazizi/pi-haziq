...
- Mesh, advisors-as-product, swarms, handoff, RLM, and councils remain unavailable unless a later approved contract enables them; casual children use Fabric agents, fleets use Dynamic Workflows.
- One job uses one multi-agent surface: casual children through Fabric `agents.*`, fleets through Dynamic Workflows. Do not dual-launch both for the same work.
