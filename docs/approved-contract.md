...
| Todo, named subagent, workflow, loop, MCP, compaction, service tier, tool-call repair, models, and Herdr share bounded correlation and diagnostics. | `extensions/haziq-cohesion.ts`, compatibility wrappers, and bundled `pi-tool-repair` | cohesion, trust, reload, Herdr, and package tests |
