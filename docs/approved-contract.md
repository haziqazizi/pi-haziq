...
| Multi-agent work uses Fabric agents and the full Fabric skill set (`fabric-guide` router); mesh is on; agent transport defaults to Herdr; Dynamic Workflows are not packaged. | Fabric pin, skill allowlist, `config/fabric.json`, `APPEND_SYSTEM.md` | manifest, setup/doctor, and package smokes |
...
| Reviewed upstream Fabric is bundled in full code mode; agents on, mesh on, transport herdr; child defaultTools include core plus packaged extension tools; all Fabric skills registered; only `fabric_exec` stays model-visible among tools. | `pi-fabric` pin, `fabric.json`, haziq-fabric | setup/doctor, captured inventory, package smokes |
...
| Herdr CLI is required for the default agent transport; Pi integration stays Herdr-managed via `herdr integration install pi`. | setup herdr ensure, cohesion doctor, public CLI only | Herdr ensure unit tests, failure and package smokes |
...
- Mesh is on by default so ambient actors/supervisors restore; Fabric swarm is available when durable multi-actor work is needed.
