...
| Dynamic workflow guidance has one visible entrypoint; the pinned runtime-owned authoring and pattern contracts remain hidden references loaded only when needed. | `designing-dynamic-workflows` Git pin and `pi.skills` allowlist | manifest and isolated package smokes |
| Fabric is bundled in full code mode with only `fabric-exec` advertised; captured extension schemas and advanced Fabric skills remain lazy or user-invoked. | exact `pi-fabric` pin and package resource allowlist | manifest, captured-inventory, and production-tarball smokes |
